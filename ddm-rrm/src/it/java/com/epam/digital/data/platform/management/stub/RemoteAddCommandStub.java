...
public class RemoteAddCommandStub extends RemoteAddCommand {
  ...
  public RemoteAddCommand setUri(URIish uri) {
    ...
    final var gerritUrl = new URIish(gerritProps.gerrit().url());
    Assertions.assertThat(uri)
        ...
        .hasFieldOrPropertyWithValue("rawPath", "/" + gerritProps.gerrit().repository())
        .hasFieldOrPropertyWithValue("user", gerritProps.gerrit().user())
        .hasFieldOrPropertyWithValue("pass", gerritProps.gerrit().password());
