...
class MasterVersionControllerIT extends BaseIT {
  ...
  public void getMasterVersionInfo(String message, String status) {
    ...
    final var lastMergedChangeInfo =
        Map.of(
            "_number",
            1,
            "owner",
            Map.of("username", context.getVcsProps().gerrit().user()),
            "topic",
            "this is description for version candidate",
            "subject",
            "commit message",
            "submitted",
            "2022-08-02 16:15:12.786589626",
            "labels",
            Map.of(),
            "messages",
            List.of(
                Map.of("message", message, "date", timestamp.toString(), "_revision_number", 1)),
            "change_id",
            "change_id");
    ...
    context.getGerritMockServer().addStubMapping(stubFor(
        WireMock.get(urlEqualTo(String.format("/a/changes/?q=project:%s+status:merged+owner:%s&n=10",
                context.getVcsProps().gerrit().repository(), context.getVcsProps().gerrit().user())))
    ...
    mockMvc.perform(
        ...
        jsonPath("$.id", is("1")),
        jsonPath("$.author", is(context.getVcsProps().gerrit().user())),
  ...
  static Stream<Arguments> provideBuildStatuses() {
    return Stream.of(
        arguments("Build Started ... MASTER-Build-registry-regulations ...", ResultValues.PENDING.name()),
        arguments("Build Successful ... MASTER-Build-registry-regulations ...", ResultValues.SUCCESS.name()),
        arguments("Build Failed ... MASTER-Build-registry-regulations ...", ResultValues.FAILED.name()),
        arguments("Build Successful ... MASTER-Code-review-registry-regulations ...", ResultValues.PENDING.name()),

        arguments("Build Started ... registry-regulations-build ...", ResultValues.PENDING.name()),
        arguments("Build Succeeded ... registry-regulations-build ...", ResultValues.SUCCESS.name()),
        arguments("Build Failed ... registry-regulations-build ...", ResultValues.FAILED.name()),
        arguments("Build Successful ... registry-regulations-code-review ...", ResultValues.PENDING.name())
    );
  }
}
