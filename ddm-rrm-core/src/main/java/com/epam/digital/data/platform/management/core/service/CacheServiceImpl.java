...
public class CacheServiceImpl implements CacheService {
  ...
  public List<String> getConflictsCache(String cacheKey) {
    Cache cache = cacheManager.getCache(CONFLICTS_CACHE_NAME);
    if (cache == null) {
  ...
  public LocalDateTime getLatestRebaseCache(String cacheKey) {
    Cache cache = cacheManager.getCache(LATEST_REBASE_CACHE_NAME);
    if (cache == null) {
      return null;
    }
    Cache.ValueWrapper valueWrapper = cache.get(cacheKey);
  ...
  public Catalog getCatalogCache(String versionId) {
    ...
    Cache.ValueWrapper valueWrapper = cache.get(versionId);
    if (Objects.nonNull(valueWrapper)) {
