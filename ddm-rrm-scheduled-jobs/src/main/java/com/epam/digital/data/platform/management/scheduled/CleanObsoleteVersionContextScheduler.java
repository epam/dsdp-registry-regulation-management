...
public class CleanObsoleteVersionContextScheduler {
  ...
  public void cleanObsoleteVersionContexts() {
    try {
      var openedMrs = vcsService.getMRList();
      var repositoriesDirectory = vcsPropertiesConfig.repositoryDirectory();

      try (var directories = Files.list(Path.of(repositoriesDirectory))) {
        directories
            .filter(path -> !path.endsWith(vcsPropertiesConfig.headBranch()))
