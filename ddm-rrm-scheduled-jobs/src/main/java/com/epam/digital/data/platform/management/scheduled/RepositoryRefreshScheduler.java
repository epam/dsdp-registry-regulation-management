...
public class RepositoryRefreshScheduler {
  ...
  public void refreshVersionCandidates() {
    ...
    var mrList = vcsService.getMRList();

    for (var changeInfo : mrList) {
      try {
        var change = vcsService.getMRByNumber(changeInfo.getNumber());
        ...
        if (conflictCache != null) {
          ...
          log.trace("Conflicts cache not found for name: {}", CONFLICTS_CACHE_NAME);
          return;
        }
        ...
        if (rebaseCache != null) {
          ...
          log.trace("Rebase cache not found for name: {}", LATEST_REBASE_CACHE_NAME);
          return;
        }
      } catch (Exception e) {
