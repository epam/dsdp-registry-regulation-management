...
class JGitServiceDeleteTest extends AbstractJGitServiceTest {
  ...
  void setUp() {
    ...
    Mockito.doReturn(VcsPropertiesConfig.VcsProvider.GERRIT).when(vcsPropertiesConfig).provider();
    Mockito.doReturn(gerritProperties).when(vcsPropertiesConfig).gerrit();
    Mockito.doReturn(REPO_URL).when(gerritProperties).url();
    Mockito.doReturn(REPO_NAME).when(gerritProperties).repository();
    Mockito.doReturn(USER).when(gerritProperties).user();
    Mockito.doReturn(PASS).when(gerritProperties).password();
    Mockito.doReturn(HEAD_BRANCH).when(vcsPropertiesConfig).headBranch();

    repoDir = Path.of(tempDir.getPath(), FilenameUtils.getName(REPO_NAME)).normalize().toFile();
    Assertions.assertThat(repoDir.mkdirs()).isTrue();
    Assertions.assertThat(
        Path.of(repoDir.getPath(), FilenameUtils.getName(FILE_PATH))
            .normalize()
            .toFile()
            .createNewFile()
    ).isTrue();
    ...
    Mockito.doReturn(pushCommand).when(pushCommand).setForce(true);
    Mockito.doReturn(pushCommand).when(pushCommand)
  ...
  void testDelete_remoteAddInvalidURI() {
    Mockito.doReturn("").when(gerritProperties).url();
    Mockito.doReturn("").when(gerritProperties).repository();
