...
class JGitServiceGetFileContentTest extends AbstractJGitServiceTest {
  ...
  void setUp() {
    ...
    File directory = Path.of(tempDir.getPath(), FilenameUtils.getName(REPO_NAME)).normalize().toFile();
    Assertions.assertThat(directory.mkdirs()).isTrue();

    file = Path.of(directory.getPath(), FilenameUtils.getName(FILE_PATH)).normalize().toFile();
  ...
  void getFileContentDirectoryNotExist() {
    ...
    Mockito.verify(jGitWrapper, never()).open(eq(Path.of(FilenameUtils.getName(path)).normalize().toFile()));
  }
