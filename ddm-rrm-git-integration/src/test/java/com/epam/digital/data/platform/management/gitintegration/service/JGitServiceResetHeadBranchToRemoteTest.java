...
class JGitServiceResetHeadBranchToRemoteTest extends AbstractJGitServiceTest {
  ...
  void setUp() {
    ...
    directory = Path.of(tempDir.getPath(), FilenameUtils.getName(REPO_NAME)).normalize().toFile();
    ...
    Mockito.doReturn(REPO_NAME).when(vcsPropertiesConfig).headBranch();
    Mockito.doReturn(USER).when(gerritProperties).user();
    Mockito.doReturn(PASSWORD).when(gerritProperties).password();
  ...
  void testResetHeadBranchToRemote_couldNotOpenRepo() {
    ...
    var file = Path.of(tempDir.toString(), FilenameUtils.getName(repoName)).normalize().toFile();
  ...
  void testResetHeadBranchToRemoteDirectoryNotExist() {
    ...
    var repo = Path.of(tempDir.toString(), FilenameUtils.getName(headBranch)).normalize().toFile();
    Assertions.assertThat(repo.exists()).isFalse();

    Mockito.when(vcsPropertiesConfig.headBranch()).thenReturn(headBranch);
