...
class JGitServiceGetFilesDatesTest extends AbstractJGitServiceTest {
  ...
  void setUp() {
    ...
    repo = Path.of(tempDir.getPath(), FilenameUtils.getName(REPOSITORY_NAME)).normalize().toFile();
  ...
  void getFormDatesTest_repoNotExist() {
    ...
    Mockito.verify(jGitWrapper, never()).open(eq(Path.of(FilenameUtils.getName(filePath)).normalize().toFile()));
  }
