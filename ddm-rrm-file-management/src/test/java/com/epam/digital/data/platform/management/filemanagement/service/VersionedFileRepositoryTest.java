...
public class VersionedFileRepositoryTest {
  ...
  @BeforeEach
  void setUp() {
    repository = new VersionedFileRepositoryImpl("version", jGitService, vcsService, mapper);
    var changeInfo = new ChangeInfoDto();
    changeInfo.setChangeId("changeId");
    changeInfo.setRefs(RandomString.make());
    Mockito.when(vcsService.getChangeInfo(changeInfo.getChangeId())).thenReturn(changeInfo);
  ...
  void getVersionedFileListTest() {
    ...
    Mockito.when(vcsService.getMRByNumber("version")).thenReturn(changeInfo);
    Mockito.when(vcsService.getListOfChangesInMR("changeId")).thenReturn(filesInMR);
  ...
  void getVersionedFileListWithStatusesTest() {
    ...
    Mockito.when(vcsService.getMRByNumber("version")).thenReturn(changeInfo);
    Mockito.when(vcsService.getListOfChangesInMR("changeId")).thenReturn(
  ...
  void pullRepositoryTest() {
    ...
    Mockito.verify(vcsService).getChangeInfo(changeInfo.getChangeId());
  ...
  @Test
  @SneakyThrows
  void rollbackFileTest() {
    var filePath = "bp-grouping/bp-grouping.yml";
    var versionId = "version";
    var changeInfo = new ChangeInfoDto();
    changeInfo.setChangeId(RandomString.make());
    changeInfo.setRefs(RandomString.make());
    Mockito.when(vcsService.getChangeInfo(changeInfo.getChangeId())).thenReturn(changeInfo);
    Mockito.when(vcsService.getMRByNumber(versionId)).thenReturn(changeInfo);

    repository.rollbackFile(filePath);

    Mockito.verify(jGitService, Mockito.times(1)).cloneRepoIfNotExist(versionId);
    Mockito.verify(vcsService).getChangeInfo(changeInfo.getChangeId());
    Mockito.verify(jGitService).fetch(versionId, changeInfo.getRefs());
    Mockito.verify(jGitService).rollbackFile(versionId, filePath);
  }
}
