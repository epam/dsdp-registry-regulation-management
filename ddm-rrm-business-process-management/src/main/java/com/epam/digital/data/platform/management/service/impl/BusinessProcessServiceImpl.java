...
import com.epam.digital.data.platform.management.utils.DocumentBuilderConfig;
...
public class BusinessProcessServiceImpl implements BusinessProcessService {
  ...
  private static final String REGISTRY_REGULATION_MANAGEMENT_URL = "http://registry-regulation-management";
  ...
  public void updateProcess(String content, String processName, String versionName, String eTag) {
    ...
    if (fileDatesDto == null) {
      log.trace("getDatesFromContent returned null for path: {}", processPath);
      return;
    }
  ...
  private Document parseBusinessProcess(String processContent) {
    try {
      return parseXml(processContent);
  ...
  private List<BusinessProcessInfoDto> getProcessesByVersion(String versionName,
    ...
    for (VersionedFileInfoDto versionedFileInfoDto : fileList) {
      if (!versionedFileInfoDto.getStatus().equals(skippedStatus)) {
        String processContent = readProcessContent(repo, masterRepo, versionedFileInfoDto);
        var dates = getDatesFromContent(processContent);
        if (dates == null) {
          continue;
        }
        fillMissingDates(repo, masterRepo, versionedFileInfoDto, dates);
        processes.add(
  ...
  private String readProcessContent(VersionedFileRepository repo,
                                    VersionedFileRepository masterRepo,
                                    VersionedFileInfoDto versionedFileInfoDto) {
    if (versionedFileInfoDto.getStatus() == FileStatus.DELETED) {
      return masterRepo.readFile(getProcessPath(versionedFileInfoDto.getName()));
    }
    return repo.readFile(getProcessPath(versionedFileInfoDto.getName()));
  }

  private void fillMissingDates(VersionedFileRepository repo,
                                VersionedFileRepository masterRepo,
                                VersionedFileInfoDto versionedFileInfoDto,
                                FileDatesDto dates) {
  ...
  private FileDatesDto getDatesFromContent(String processContent) {
    Document doc;
    try {
      doc = parseXml(processContent);
    } catch (SAXException | IOException | ParserConfigurationException exception) {
      log.warn("Exception during processing xml document", exception);
      return null;
    }
    doc.getDocumentElement().normalize();
  ...
  private String addDatesToContent(String processContent, LocalDateTime created,
    ...
    element.setAttributeNS(
        "http://www.w3.org/2000/xmlns/", "xmlns:rrm", REGISTRY_REGULATION_MANAGEMENT_URL);
    element.setAttributeNS(
        REGISTRY_REGULATION_MANAGEMENT_URL,
  ...
  private String getStringFromDocument(Document doc) {
    try {
      ...
      Transformer transformer = SecureXMLFactory.transformer();
  ...
  /** FalsePositive
   * Sonar reports this because it cannot track how the
   * DocumentBuilderFactory is configured across class boundaries. This method creates a
   * DocumentBuilder with full XXE protections: DOCTYPE declarations are disallowed, external
   * general/parameter entities are disabled, external DTD loading is blocked, XInclude is disabled,
   * and entity reference expansion is turned off. The parser is safe against XXE attacks, but the
   * analyzer cannot detect this due to inter-procedural analysis limitations.
   */
  @SuppressWarnings("findsecbugs:XXE_DOCUMENT")
  private static Document parseXml(String xmlContent)
      throws ParserConfigurationException, IOException, SAXException {
    return DocumentBuilderConfig.documentBuilder()
