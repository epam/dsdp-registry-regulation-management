...
public class CandidateVersionSettingsController {
  ...
  @Operation(
      summary = "Rollback global settings file within version-candidate",
      description = "### Endpoint purpose:\n This endpoint is used for rolling back the __settings file__ from the __version-candidate__. It is intended for situations where the __settings file__ needs to be reverted to a prior version, such as to mitigate data corruption or to restore a previous state.",
      parameters = @Parameter(
          in = ParameterIn.HEADER,
          name = "X-Access-Token",
          description = "Token used for endpoint security",
      ...
      responses = {
          @ApiResponse(
              responseCode = "200",
              description = "OK. Settings file successfully rolled back.",
              content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE)
          ),
          @ApiResponse(
              responseCode = "401",
  ...
  @PostMapping("/{settingsFileName}/rollback")
  public ResponseEntity<String> rollbackForm(
      ...
      @PathVariable @Parameter(description = "Name of the settings file to be rolled back", required = true) String settingsFileName) {
    log.info("Started rolling back {} file from {} version candidate", settingsFileName, versionCandidateId);
    settingService.rollbackSettingsFile(settingsFileName, versionCandidateId);
    log.info("Finished rolling back {} file from the {} version candidate", settingsFileName, versionCandidateId);
