...
public class ApplicationExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  public static final String BUSINESS_PROCESS_ALREADY_EXISTS_EXCEPTION =
      "BUSINESS_PROCESS_ALREADY_EXISTS_EXCEPTION";
  public static final String TABLE_NOT_FOUND_EXCEPTION = "TABLE_NOT_FOUND_EXCEPTION";
  public static final String REGISTRY_DATA_BASE_CONNECTION_ERROR =
      "REGISTRY_DATA_BASE_CONNECTION_ERROR";
  ...
  public static final String GROUPS_PROCESS_DEFINITION_EXCEPTION =
      "GROUPS_PROCESS_DEFINITION_EXCEPTION";
  public static final String GROUPS_PROCESS_DEFINITION_DUPLICATES_EXCEPTION =
  ...
  private static final String BUSINESS_PROCESS_CONTENT_EXCEPTION =
      "BUSINESS_PROCESS_CONTENT_EXCEPTION";
  private static final String INVALID_VERSION_CANDIDATE_EXCEPTION =
      "INVALID_VERSION_CANDIDATE_EXCEPTION";
  ...
  private static final String REDASH_ENTITY_NOT_FOUND = "REDASH_ENTITY_NOT_FOUND";
  private static final String REDASH_DASHBOARD_ZIPPING_EXCEPTION =
      "REDASH_DASHBOARD_ZIPPING_EXCEPTION";
  ...
  public ResponseEntity<Object> handleMaxUploadSizeExceededException(
      @NotNull MaxUploadSizeExceededException exception, @NotNull HttpHeaders headers,
  ...
  public ResponseEntity<DetailedErrorResponse> handleGerritChangeNotFoundException(
      VcsChangeNotFoundException exception) {
  ...
  public ResponseEntity<DetailedErrorResponse> handleGerritCommunicationException(
      VcsCommunicationException exception) {
  ...
  public ResponseEntity<DetailedErrorResponse> handleGerritConflictException(
      VcsConflictException exception) {
  ...
  @ExceptionHandler
  public ResponseEntity<DetailedErrorResponse> handleI18nNotFoundException(
      I18nNotFoundException exception) {
    log.error("I18n bundle {} not found", exception.getI18nName());
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(newDetailedResponse(FileValidatorErrorMessageTitle.I18N_NOT_FOUND,
            Map.of("bundleName", exception.getI18nName()), exception));
  }

  @ExceptionHandler
  public ResponseEntity<DetailedErrorResponse> handleI18nAlreadyExistsException(
      I18nAlreadyExistsException exception) {
    log.error("I18n bundle {} already exists", exception.getI18nName());
    return ResponseEntity.status(HttpStatus.CONFLICT)
        .body(newDetailedResponse(FileValidatorErrorMessageTitle.I18N_ALREADY_EXISTS,
            Map.of("bundleName", exception.getI18nName()), exception));
  }
  ...
  @ExceptionHandler(RedashEntityNotFoundException.class)
  public ResponseEntity<DetailedErrorResponse> handleNotFoundException(
      RedashEntityNotFoundException exception) {
    log.error("Redash object not found", exception);
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(newDetailedResponse(REDASH_ENTITY_NOT_FOUND, exception));
  }

  @ExceptionHandler(RedashDashboardZippingException.class)
  public ResponseEntity<DetailedErrorResponse> handleDashboardZippingException(
      RedashDashboardZippingException exception) {
    log.error("Error during zipping redash dashboard with queries", exception);
    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
        .body(newDetailedResponse(REDASH_DASHBOARD_ZIPPING_EXCEPTION, exception));
  }
  ...
  private DetailedErrorResponse newDetailedResponse(String code, Exception exception) {
    return newDetailedResponse(code, null, exception);
  }

  private DetailedErrorResponse newDetailedResponse(String code,
      Map<String, String> messageParameters, Exception exception) {
    var response = new DetailedErrorResponse();
    response.setTraceId(getTraceId());
    response.setCode(code);
    response.setDetails(exception.getMessage());

    var title = FileValidatorErrorMessageTitle.from(code);
    response.setMessageKey(Objects.isNull(title) ? null : title.getMessageKey());
    response.setMessageParameters(messageParameters);

    return response;
  }

  private DetailedErrorResponse newDetailedResponse(FileValidatorErrorMessageTitle title,
      Map<String, String> messageParameters, Exception exception) {
    var response = new DetailedErrorResponse();
    response.setTraceId(getTraceId());
    response.setCode(title.getErrorCode());
    response.setDetails(exception.getMessage());
    response.setMessageKey(title.getMessageKey());
    response.setMessageParameters(messageParameters);
  ...
  private String getTraceId() {
    return Optional.ofNullable(tracer.currentSpan()).map(Span::context).map(ctx -> ctx.traceId())
        .orElse(null);
  }
}
