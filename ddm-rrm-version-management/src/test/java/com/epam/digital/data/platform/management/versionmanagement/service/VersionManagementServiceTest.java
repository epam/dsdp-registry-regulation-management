...
class VersionManagementServiceTest extends VersionManagementServiceBaseTest {
  ...
  void getVersionsListTest() {
    ...
    Mockito.when(vcsService.getMRList()).thenReturn(List.of(changeInfo));
  ...
  void declineTest() {
    ...
    Mockito.doNothing().when(vcsService).declineChange(changeId);
    Assertions.assertThatCode(() -> managementService.decline(changeId))
        .doesNotThrowAnyException();

    Mockito.verify(vcsService).declineChange(changeId);
  }
  ...
  void markReviewedTest() {
    var changeId = RandomString.make();
    Mockito.when(vcsService.review(changeId)).thenReturn(true);

    Assertions.assertThatCode(() -> managementService.markReviewed(changeId))
        .doesNotThrowAnyException();
    Mockito.verify(vcsService).review(changeId);
  ...
  void getVersionFileListTest() {
    ...
    Mockito.when(vcsService.getListOfChangesInMR("3"))
  ...
  void createNewVersionTest() {
    ...
    final var createChangeInputDto = CreateChangeInputDto.builder()
        ...
        .build();
    Mockito.when(vcsService.createChanges(createChangeInputDto)).thenReturn(versionNumber);
    ...
    Mockito.verify(vcsService).createChanges(createChangeInputDto);
  ...
  void getVersionDetailsTest() {
    ...
    Mockito.when(vcsService.getMRByNumber("1")).thenReturn(changeInfo);
  ...
  void getVersionDetailsTest_notFound() {
    ...
    Mockito.when(vcsService.getMRByNumber(changeNumber)).thenReturn(null);
  ...
  void getMasterInfo() {
    ...
    Mockito.when(vcsService.getLastMergedMR()).thenReturn(changeInfo);
  ...
  void getMasterInfo_null() {
    Mockito.when(vcsService.getLastMergedMR()).thenReturn(null);

    var result = managementService.getMasterInfo();

    Assertions.assertThat(result)
        .isNull();
    Mockito.verify(vcsService).getLastMergedMR();
  }
  ...
  void getVersionChanges() {
    ...
    changeInfo.setMergeable(true);
    changeInfo.setLabels(Map.of("label1", -1, "label2", -1));
    Mockito.when(vcsService.getMRByNumber(version)).thenReturn(changeInfo);
  ...
  void rebaseTest() {
    ...
    Mockito.doNothing().when(vcsService).rebase(changeId);
    ...
    Mockito.when(vcsService.getChangeInfo(changeId)).thenReturn(changeInfoDto);

    Mockito.doNothing().when(jGitService).fetch(version, refs);

    managementService.rebase(version);

    Mockito.verify(vcsService).getMRByNumber(version);
    Mockito.verify(vcsService).rebase(changeId);
    Mockito.verify(vcsService).getChangeInfo(changeId);
