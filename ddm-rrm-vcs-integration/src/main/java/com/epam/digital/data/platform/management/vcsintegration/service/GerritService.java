...
public class GerritService implements VcsService {
  ...
  public List<ChangeInfoShortDto> getMRList() {
    ...
    try {
      ...
      if (ex.getStatusCode() == HttpStatus.NOT_FOUND.value()) {
        throw new VcsChangeNotFoundException("Could not found candidates", ex);
      } else {
        throw new VcsCommunicationException("Something went wrong wile getting candidates", ex);
      }
  ...
  public ChangeInfoDto getLastMergedMR() {
    ...
    try {
      ...
      var optionalChangeId = changeInfoList.stream()
          .max(Comparator.comparing(changeInfo -> changeInfo.submitted))
          .map(changeInfo -> changeInfo.changeId);
      if (optionalChangeId.isEmpty()) {
        return null;
      }
      return gerritMapper.toChangeInfoDto(changes.id(optionalChangeId.get()).get());
    } catch (HttpStatusException ex) {
      if (ex.getStatusCode() == HttpStatus.NOT_FOUND.value()) {
        throw new VcsChangeNotFoundException("Could not found last merged change", ex);
      } else {
        throw new VcsCommunicationException(
            "Something went wrong wile getting last merged change", ex);
      }
    } catch (RestApiException ex) {
      throw new VcsCommunicationException("Something went wrong wile getting last merged change",
          ex);
    }
  }
  ...
  public List<String> getClosedMrIds() {
    ...
    try {
      ...
      if (ex.getStatusCode() == HttpStatus.NOT_FOUND.value()) {
        throw new VcsChangeNotFoundException("Could not found any closed candidate", ex);
      } else {
        throw new VcsCommunicationException(
  ...
  public ChangeInfoDto getMRByNumber(String number) {
    String query = String.format("project:%s+%s", vcsPropertiesConfig.gerrit().repository(), number);
    Changes changes = gerritApi.changes();

    try {
      var changeInfos = changes.query(query).get();
      if (changeInfos.isEmpty()) {
        throw new VcsChangeNotFoundException("Could not get change info for " + number + " MR");
      }
      ...
      if (ex.getStatusCode() == HttpStatus.NOT_FOUND.value()) {
        throw new VcsChangeNotFoundException("Could not found candidate with number " + number,
            ex);
      } else {
        throw new VcsCommunicationException(
  ...
  public ChangeInfoDto getChangeInfo(String changeId) {
    try {
      ...
      if (ex.getStatusCode() == HttpStatus.NOT_FOUND.value()) {
        throw new VcsChangeNotFoundException("Could not found candidate with id " + changeId,
            ex);
      } else {
        throw new VcsCommunicationException(
  ...
  public Map<String, FileInfoDto> getListOfChangesInMR(String changeId) {
    try {
      ...
      if (e.getStatusCode() == HttpStatus.NOT_FOUND.value()) {
        throw new VcsChangeNotFoundException("Could not found candidate with id " + changeId, e);
      }
    } catch (RestApiException ex) {
      throw new VcsCommunicationException(
  ...
  public void submitChanges(String changeId) {
    try {
      ...
      if (ex.getStatusCode() == HttpStatus.CONFLICT.value()) {
        throw new VcsConflictException(
  ...
  public String createChanges(CreateChangeInputDto createDto) {
    ...
    changeInput.project = vcsPropertiesConfig.gerrit().repository();
    changeInput.branch = vcsPropertiesConfig.headBranch();
    AccountInput accountInput = new AccountInput();
    accountInput.username = vcsPropertiesConfig.gerrit().user();
    accountInput.name = vcsPropertiesConfig.gerrit().user();
    accountInput.email = vcsPropertiesConfig.gerrit().user();
    accountInput.httpPassword = vcsPropertiesConfig.gerrit().password();
    ...
    try {
      ...
      if (ex.getStatusCode() == HttpStatus.NOT_FOUND.value()) {
        throw new VcsChangeNotFoundException(
  ...
  public Boolean review(String changeId) {
    ...
    reviewInput.reviewer(vcsPropertiesConfig.gerrit().user(), ReviewerState.REVIEWER, true);
