...
package com.epam.digital.data.platform.management.reportexporter.config.feign;
...
public class FeignErrorDecoder implements ErrorDecoder {
  ...
  public Exception decode(String methodKey, Response response) {
    ...
    log.error(
        "Error during call to Redash, request url {}, status code {}, response body: {}",
        response.request().url(),
        response.status(),
        response.body());
    if (status.equals(HttpStatus.NOT_FOUND)) {
      throw new RedashEntityNotFoundException("Redash object not found");
    }
